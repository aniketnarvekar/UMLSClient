...
>: UMLSSourceVocabularyInformation, Decodable, Sendable {
  ...
  public init(from decoder: any Decoder) throws {
    ...
    guard let abbreviation = try container.decodeNoneString(forKey: .abbreviation) else {
      throw DecodingError.dataCorruptedError(
        forKey: .abbreviation, in: container,
        debugDescription: "Invalid abbreviation \"NONE\" or \"\".")
    }
    ...
    guard let expandedForm = try container.decodeNoneString(forKey: .expandedForm) else {
      throw DecodingError.dataCorruptedError(
        forKey: .expandedForm, in: container,
        debugDescription: "Invalid expandedForm \"NONE\" or \"\".")
    }
    ...
    guard let family = try container.decodeNoneString(forKey: .family) else {
      throw DecodingError.dataCorruptedError(
        forKey: .family, in: container, debugDescription: "Invalid family \"NONE\" or \"\".")
    }
    ...
    self.acquisitionContact = try container.decodeNoneString(forKey: .acquisitionContact)?
      .trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    guard let shortName = try container.decodeNoneString(forKey: .shortName) else {
      throw DecodingError.dataCorruptedError(
        forKey: .shortName, in: container, debugDescription: "Invalid shortName \"NONE\" or \"\".")
    }
    self.shortName = shortName.trimmingCharacters(in: .whitespacesAndNewlines)

    self.hierarchicalName = try container.decodeNoneString(forKey: .hierarchicalName)?
      .trimmingCharacters(in: .whitespacesAndNewlines)
    guard let preferredName = try container.decodeNoneString(forKey: .preferredName) else {
      throw DecodingError.dataCorruptedError(
        forKey: .preferredName, in: container,
        debugDescription: "Invalid preferredName \"NONE\" or \"\".")
    }
